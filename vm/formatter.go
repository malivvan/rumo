...
func (p *pp) printArg(arg Object, verb rune) {
	...
	case *Float32:
		p.fmtFloat(float64(f.Value), 32, verb)
	case *Float64:
	...
	case *Int16:
		p.fmtInteger(uint64(f.Value), signed, verb)
	case *Byte:
		p.fmtInteger(uint64(int8(f.Value)), signed, verb)
	case *Uint8:
		p.fmtInteger(uint64(f.Value), unsigned, verb)
	case *Uint16:
		p.fmtInteger(uint64(f.Value), unsigned, verb)
	case *Uint:
		p.fmtInteger(uint64(f.Value), unsigned, verb)
	case *Uint64:
		p.fmtInteger(f.Value, unsigned, verb)
	case *Char:
		p.fmtInteger(uint64(f.Value), signed, verb)
	case *String:
