...
NewApp documentation for usage examples.
...
Widgets must implement the Widget interface. All widgets embed the base
widget, Box, and thus inherit its functions. This isn't necessarily
...
Widgets may be used without an application created via NewApp, allowing
...
Setting the background color of a widget to tcell.ColorDefault will use the
default terminal background color. To enable transparency (allowing one or more
widgets to display behind a widget) call SetBackgroundTransparent. The
...
When widgets are instantiated, they are initialized with colors taken from
the global Styles variable. You may change this variable to adapt the look and
feel of the widgets to your preferred style.
...
application, set the TextView as its root widget, and run the event loop.
The application exits when the application's Stop() function is called or when
Ctrl-C is pressed.

If we have a widget which consumes key presses, we call the application's
SetFocus() function to redirect all key presses to that widget. Most
widgets then offer ways to install handlers that allow you to react to any
