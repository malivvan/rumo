...
// gridItem represents one widget and its possible position on a grid.
...
// size of the rows and columns, then placing widgets into the grid.
//
...
// and "l" keys) while the grid has focus and none of its contained widgets
// do.
type Grid struct {
	*Box
	...
	// The size of the gaps between neighboring widgets. This is automatically
...
// areas not covered by any widgets will show widgets behind the Grid.
...
//
// Widgets may extend beyond the columns defined explicitly with this
// function. A value of 0 is assumed for any undefined column. In fact, if you
// never call this function, all columns occupied by widgets will have the
...
//
// If a widget were then placed in the 6th and 7th column, the resulting
...
// of the widget will be located in the top-left corner of the grid cell at
// the given row and column and will span "rowSpan" rows and "colSpan" columns.
// For example, for a widget to occupy rows 2, 3, and 4 and columns 5 and 6:
//
//	grid.AddItem(p, 2, 5, 3, 2, 0, 0, true)
//
// If rowSpan or colSpan is 0, the widget will not be drawn.
//
// You can add the same widget multiple times with different grid positions.
...
// widget apply, the one that has at least one highest minimum value will be
// used, or the widget added last if those values are the same. Example:
//
...
// RemoveItem removes all items for the given widget from the grid, keeping
...
func (g *Grid) InputHandler() func(event *tcell.EventKey, setFocus func(w Widget)) {
	return g.WrapInputHandler(func(event *tcell.EventKey, setFocus func(w Widget)) {
		g.Lock()
		defer g.Unlock()
...
func (g *Grid) Draw(screen tcell.Screen) {
	...
	for widget, item := range items {
		...
		item.visible = true
		if widget.GetFocusable().HasFocus() {
			focus = item
		}
	}
	...
	for widget, item := range items {
		...
		item.x += x
		item.y += y
		widget.SetRect(item.x, item.y, item.w, item.h)

		// Draw widget.
		if item == focus {
			defer widget.Draw(screen)
		} else {
			widget.Draw(screen)
		}
...
func (g *Grid) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return g.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
