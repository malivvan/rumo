...
func (h *HexView) Draw(screen tcell.Screen) {
	...
	if pageDown {
		...
		rowOffset = max(rowOffset-height, 0)
	}

	// don't allow overscroll
	if rowOffset > rows-height {
		rowOffset = max(rows-height, 0)
	}
	...
	for row := 0; row < height; row++ {
		...
		indexOffset = (row + rowOffset) * 16
		baseY = y + row

		// print offset, 8 hex digits
		Print(screen, []byte(fmt.Sprintf("%08x |", indexOffset)), baseX, baseY, offsetWidth, AlignLeft, tcell.ColorDarkGray)
		...
		for j := 0; j < 8; j++ {
			...
			// hex pairs
			Print(screen, []byte(fmt.Sprintf(" %02x", data[index])), baseX+offsetWidth+(j*byteWidth), baseY, byteWidth, AlignLeft, colorize(data[index]))
			// ascii
			Print(screen, []byte(h.printableCharacter(data[index])), baseX+offsetWidth+4+j+(16*byteWidth), baseY, byteWidth, AlignLeft, colorize(data[index]))
		}

		// divider: byte blocks
		Print(screen, []byte(" ┊"), baseX+offsetWidth+(byteWidth*8), baseY, 2, AlignLeft, tcell.ColorWhite)
		...
		for j := 8; j < 16; j++ {
			...
			// hex pairs
			Print(screen, []byte(fmt.Sprintf(" %02x", data[index])), baseX+offsetWidth+2+(j*byteWidth), baseY, byteWidth, AlignLeft, colorize(data[index]))
			// ascii
			Print(screen, []byte(h.printableCharacter(data[index])), baseX+offsetWidth+5+j+(16*byteWidth), baseY, byteWidth, AlignLeft, colorize(data[index]))
		}

		// divider: byte blocks from ascii
		Print(screen, []byte(" |"), baseX+offsetWidth+2+(byteWidth*16), baseY, 2, AlignLeft, tcell.ColorWhite)
		// divider: ascii blocks
		Print(screen, []byte("┊"), baseX+offsetWidth+12+(byteWidth*16), baseY, 1, AlignLeft, tcell.ColorWhite)
		// divider: ascii close
		Print(screen, []byte("|"), baseX+offsetWidth+20+(byteWidth*16), baseY, 1, AlignLeft, tcell.ColorWhite)
	}
}
...
func (h *HexView) InputHandler() func(event *tcell.EventKey, setFocus func(w Widget)) {
	return h.WrapInputHandler(func(event *tcell.EventKey, setFocus func(w Widget)) {
		h.Lock()
		defer h.Unlock()
...
func (h *HexView) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return h.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
		return false, nil
	})
}
