...
func (vt *VT) cup(pm []int) {
	...
	top := row(0)
	bottom := row(vt.height() - 1)
	left := column(0)
	right := column(vt.width() - 1)
	if vt.mode&decom != 0 {
		top = vt.margin.top
		bottom = vt.margin.bottom
		left = vt.margin.left
		right = vt.margin.right
	}

	vt.cursor.row = top + row(r-1)
	vt.cursor.col = left + column(c-1)
	if vt.cursor.col > right {
		vt.cursor.col = right
	}
	if vt.cursor.col < left {
		vt.cursor.col = left
	}
	if vt.cursor.row > bottom {
		vt.cursor.row = bottom
	}
	if vt.cursor.row < top {
		vt.cursor.row = top
	}
}
