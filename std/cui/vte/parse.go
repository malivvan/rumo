...
func (p *Parser) setExit(f func()) {
	p.exitMu.Lock()
	p.exit = f
	p.exitMu.Unlock()
}

func (p *Parser) runExit() {
	p.exitMu.Lock()
	exit := p.exit
	p.exit = nil
	p.exitMu.Unlock()
...
// SetExitFunc replaces the parser exit callback.
// It is primarily useful for tests that need to observe exit handling.
