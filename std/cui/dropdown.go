...
func (d *DropDownOption) getSelectedFunc() func(index int, option *DropDownOption) {
	d.RLock()
	defer d.RUnlock()
...
func NewDropDown() *DropDown {
	...
	list.SetSelectedTextColor(Styles.WidgetBackgroundColor)
...
func (d *DropDown) SetCurrentOption(index int) {
	...
	var optionSelected func(index int, option *DropDownOption)
	if index >= 0 && index < len(d.options) {
		d.currentOption = index
		d.list.SetCurrentItem(index)
		option = d.options[index]
		optionSelected = option.getSelectedFunc()
	} else {
		d.currentOption = -1
...
func (d *DropDown) SetTextOptions(prefix, suffix, currentPrefix, currentSuffix, noSelection string) {
	...
	for index := 0; index < d.list.GetItemCount(); index++ {
		d.list.SetItemText(index, prefix+d.options[index].GetText()+suffix, "")
	}
}
...
func (d *DropDown) getFieldWidth() int {
	...
	for _, option := range d.options {
		width := TaggedStringWidth(option.GetText())
		if width > fieldWidth {
			fieldWidth = width
		}
	}
...
func (d *DropDown) addOptions(options ...*DropDownOption) {
	...
	for _, option := range options {
		d.list.AddItem(NewListItem(d.optionPrefix + option.GetText() + d.optionSuffix))
	}
}
...
func (d *DropDown) Draw(screen tcell.Screen) {
	...
	for _, option := range d.options {
		strWidth := TaggedStringWidth(option.GetText()) + optionWrapWidth
	...
	if fieldWidth == 0 {
		...
		if d.currentOption < 0 {
			...
			currentOptionWidth := TaggedStringWidth(d.currentOptionPrefix + d.options[d.currentOption].GetText() + d.currentOptionSuffix)
	...
	if d.open && len(d.prefix) > 0 {
		...
		listItemText := d.options[d.list.GetCurrentItemIndex()].GetText()
		...
		text := d.noSelection
		if d.currentOption >= 0 && d.currentOption < len(d.options) {
			text = d.currentOptionPrefix + d.options[d.currentOption].GetText() + d.currentOptionSuffix
		}
...
// openList hands control over to the embedded List widget.
...
func (d *DropDown) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return d.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
