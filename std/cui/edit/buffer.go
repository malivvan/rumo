...
import (
	...
	"github.com/malivvan/rumo/std/cui/edit/highlight"
)
...
func NewBuffer(reader io.Reader, size int64, path string, cursorPosition []string) *Buffer {
	b := new(Buffer)
	var fileformat int
	b.LineArray, fileformat = NewLineArray(uint64(size), reader)
	...
	if fileformat == lineEndingLF {
		b.Settings["fileformat"] = "unix"
	} else if fileformat == lineEndingCRLF {
...
func (b *Buffer) updateRules(runtimeFiles *RuntimeFiles) {
	...
	for _, f := range runtimeFiles.ListRuntimeFiles(RTSyntax) {
		...
		if err == nil {
			...
			header, err := highlight.ParseHeader(data)
			if err != nil {
				continue
			}
