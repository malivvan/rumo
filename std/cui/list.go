...
func NewList() *List {
	l := &List{
		...
		selectedTextColor:       Styles.WidgetBackgroundColor,
	...
	for index, item := range l.items {
		...
		if item.disabled {
			...
			if showShortcuts && item.shortcut != 0 {
				Print(screen, []byte(fmt.Sprintf("(%c)", item.shortcut)), x-5, y, 4, AlignRight, color.DarkSlateGray)
			}

			// Main text.
			Print(screen, mainText, x, y, width, AlignLeft, color.Gray)
...
func (l *List) InputHandler() func(event *tcell.EventKey, setFocus func(w Widget)) {
	return l.WrapInputHandler(func(event *tcell.EventKey, setFocus func(w Widget)) {
		l.Lock()
...
func (l *List) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return l.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
		l.Lock()
