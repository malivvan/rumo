...
// widgets are arranged horizontally or vertically. The way they are
...
// NewFlex returns a new flexbox layout container with no widgets and its
// direction set to FlexColumn. To add widgets to this layout, see AddItem().
// To change the direction, see SetDirection().
//
// Note that Flex will have a transparent background by default so that any nil
// flex items will show widgets behind the Flex.
...
// GetDirection returns the direction in which the contained widgets are
...
// widget receives focus. If multiple items have the "focus" flag set to
...
// RemoveItem removes all items for the given widget from the container,
...
func (f *Flex) RemoveItem(w Widget) {
	...
	for index := len(f.items) - 1; index >= 0; index-- {
		if f.items[index].Item == w {
			f.items = append(f.items[:index], f.items[index+1:]...)
		}
	}
}

// ResizeItem sets a new size for the item(s) with the given widget. If there
// are multiple Flex items with the same widget, they will all receive the
