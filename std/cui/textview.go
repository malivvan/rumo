...
//     this widget or any other widget should be queued using
//     App.QueueUpdate().
//
...
func (t *TextView) Focus(delegate func(w Widget)) {
	...
	// Implemented here with locking because this is used by layout widgets.
	t.hasFocus = true
}
...
func (t *TextView) Draw(screen tcell.Screen) {
	...
	// Draw the buffer.
	defaultStyle := tcell.StyleDefault.Foreground(t.textColor).Background(backgroundColor)
		...
		if drawAtY >= 0 {
			...
			iterateString(string(strippedText), func(main rune, comb []rune, textPos, textWidth, screenPos, screenWidth int) bool {
				...
				if highlighted {
					...
					if (fg == tcell.ColorDefault || fg == color.None) && (bg == tcell.ColorDefault || bg == color.None) {
...
func (t *TextView) InputHandler() func(event *tcell.EventKey, setFocus func(w Widget)) {
	return t.WrapInputHandler(func(event *tcell.EventKey, setFocus func(w Widget)) {
		key := event.Key()
...
func (t *TextView) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return t.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
		x, y := event.Position()
		if !t.InRect(x, y) {
			return false, nil
		}
