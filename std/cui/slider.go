...
func (s *Slider) InputHandler() func(event *tcell.EventKey, setFocus func(w Widget)) {
	return s.WrapInputHandler(func(event *tcell.EventKey, setFocus func(w Widget)) {
...
func (s *Slider) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return s.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
		x, y := event.Position()
		if !s.InRect(x, y) {
			s.dragging = false
			return false, nil
