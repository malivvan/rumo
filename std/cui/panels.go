...
type panel struct {
	...
	Item    Widget // The panel's widget.
	Resize  bool   // Whether or not to resize the panel when it is drawn.
	Visible bool   // Whether or not this panel is visible.
}

// Panels is a container for other widgets often used as the application's
// root widget. It allows to easily switch the visibility of the contained
// widgets.
type Panels struct {
	*Box
...
// AddPanel adds a new panel with the given name and widget. If there was
...
// widget will be set to the size available to the Panels widget whenever
...
func (p *Panels) MouseHandler() func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
	return p.WrapMouseHandler(func(action MouseAction, event *tcell.EventMouse, setFocus func(w Widget)) (consumed bool, capture Widget) {
...
// AddPage adds a new panel with the given name and widget.
