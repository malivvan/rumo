...
// TestDeadcodeElimination checks that a simple program using cli in its
...
// reachable.
// Since cli can use text/template templates this test checks that in its
// default configuration that code path can be proven to be unreachable by
// the linker.
//
// See also: https://github.com/malivvan/rumo/std/cli/pull/1956
func TestDeadcodeElimination(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("go tool nm fails on windows")
	}

	// check that a simple program using cli in its default configuration is
