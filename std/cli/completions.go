...
type CompletionOptions struct {
	// DisableDefaultCmd prevents Cli from creating a default 'completion' command
	DisableDefaultCmd bool
	// DisableNoDescFlag prevents Cli from creating the '--no-descriptions' flag
...
func (c *Command) initCompleteCmd(args []string) {
	...
	if err != nil || subCmd.Name() != ShellCompRequestCmd {
		...
		// cli program that only consists of the root command, since this
...
func (c *Command) getCompletions(args []string) (*Command, []Completion, ShellCompDirective, error) {
	...
	if flag == nil && len(toComplete) > 0 && toComplete[0] == '-' && !strings.Contains(toComplete, "=") && flagCompletion {
		...
		if len(completions) == 0 {
			...
			// This allows programs to tell Cli about flags for completion even
			// if the actual parsing of flags is not done by Cli.
		...
		if !finalCmd.DisableFlagParsing {
			// If DisableFlagParsing==false, we have completed the flags as known by Cli;
			// we can return what we found.
			// If DisableFlagParsing==true, Cli may not be aware of all flags, so we
...
func helpOrVersionFlagPresent(cmd *Command) bool {
	if versionFlag := cmd.Flags().Lookup("version"); versionFlag != nil &&
		len(versionFlag.Annotations[FlagSetByCliAnnotation]) > 0 && versionFlag.Changed {
		return true
	}
	if helpFlag := cmd.Flags().Lookup(helpFlagName); helpFlag != nil &&
		len(helpFlag.Annotations[FlagSetByCliAnnotation]) > 0 && helpFlag.Changed {
		return true
	}
	return false
}
	...
	if len(lastArg) > 0 && lastArg[0] == '-' {
		if index := strings.Index(lastArg, "="); index >= 0 {
			...
			if strings.HasPrefix(lastArg[:index], "--") {
				...
				// https://github.com/malivvan/rumo/std/cli/issues/1257
