...
// ReadPassword reads a line from the configured input source using a suitable Config for password input (with masking enabled), allowing inline editing. The returned error is either nil, io.EOF, or shell.ErrInterrupt.
...
// The returned error is either nil, io.EOF, or shell.ErrInterrupt.
...
// ReadLineWithDefault is a convenience method that combines SetDefault and ReadLine, allowing you to read a line with a prefilled default value. The returned error is either nil, io.EOF, or shell.ErrInterrupt.
...
// Close closes the shell instance, cleaning up state changes to the
...
// CaptureExitSignal registers handlers for common exit signals that will close the shell instance.
...
func (i *Instance) ClearScreen() {
	_ = clearScreen(i.operation.Stdout())
}
