...
// CompleteRefresh is used for complete-mode and select-mode
func (o *opCompleter) CompleteRefresh() {
	...
	_, _ = buf.Write(bytes.Repeat([]byte("\n"), lineCnt)) // move down from cursor to start of candidates
	_, _ = buf.WriteString("\033[J")
	...
	for ; idx < len(o.candidate); idx++ {
		...
		if tWidth > 0 {
			...
			if platform.IsWindows && inSelect {
				sWidth = 1 // adjust for highlighting on Windows
			}
		...
		if lines > 0 && colIdx == 0 {
			...
			// line at the end and stealing real estate.
			_, _ = buf.WriteString("\n")
		}

		if inSelect {
			_, _ = buf.WriteString("\033[30;47m")
		}

		_, _ = buf.WriteString(string(same))
		_, _ = buf.WriteString(string(c))
		if o.candidateColNum >= 1 {
			// only output spaces between columns if everything fits
			_, _ = buf.Write(bytes.Repeat([]byte(" "), o.candidateColWidth-cWidth))
		}
	...
	_, _ = fmt.Fprintf(buf, "\033[%dA", lines)
	_, _ = buf.Write(o.op.buf.getBackspaceSequence())
	_ = buf.Flush()
}
...
func (o *opCompleter) initPage() {
	...
	o.linesAvail = tHeight - bufLineCnt - 1 // lines available without scrolling buffer offscreen, reserve one line for the guidance message
...
func (o *opCompleter) ExitCompleteMode(revert bool) {
