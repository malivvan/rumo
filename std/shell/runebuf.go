...
func (r *runeBuffer) getBackspaceSequence() []byte {
	...
	for _, r := range sp[spi] {
		if bcnt >= 0 {
			break
		}
		column += runes.Width(r)
		bcnt++
	}

	buf := bytes.NewBuffer(nil)
	if up > 0 {
		_, _ = fmt.Fprintf(buf, "\033[%dA", up) // move cursor up to index line
	}
	_, _ = fmt.Fprintf(buf, "\033[%dG", column) // move cursor to column
...
func (r *runeBuffer) SetStyle(start, end int, style string) {
	...
	move := start - r.idx
	if move > 0 {
		_, _ = r.w.Write([]byte(string(r.buf[r.idx : r.idx+move])))
	} else {
		_, _ = r.w.Write(bytes.Repeat([]byte("\b"), r.calWidth(move)))
	}
	_, _ = r.w.Write([]byte("\033[" + style + "m"))
	_, _ = r.w.Write([]byte(string(r.buf[start:end])))
	_, _ = r.w.Write([]byte("\033[0m"))
...
func (r *runeBuffer) cleanOutput(w io.Writer, idxLine int) {
	...
	tWidth, _ := r.w.GetWidthHeight()
	if tWidth == 0 {
		_, _ = buf.WriteString(strings.Repeat("\r\b", len(r.buf)+r.promptLen()))
		_, _ = buf.Write([]byte("\033[J"))
	} else {
		if idxLine > 0 {
			_, _ = fmt.Fprintf(buf, "\033[%dA", idxLine) // move cursor up by idxLine
		}
		_, _ = fmt.Fprintf(buf, "\033[%dG", r.ppos+1) // move cursor back to initial ppos position
		_, _ = buf.Write([]byte("\033[J"))            // clear from cursor to end of screen
	}
	_ = buf.Flush()
	return
}
